...
export function interp(sp: SExpr[]): Value[] {
  ...
  function evalEExprL(sl: SExpr[], localVars: Values): Value {
    ...
    if (typeof f === "string") {
      ...
      if (f.endsWith("?")) {
        ...
        if (["boolean?", "string?", "number?", "function?"].includes(f)) {
          return (
            typeof evalSExpr(elems[0], localVars) ===
            (f === "function?" ? "object" : f.slice(0, -1))
          );
        }
        if (f === "true?") {
      ...
      if (f === "lambda") {
        ...
        if (
          !(
            Array.isArray(args) &&
            args.every<string | TypedVar>((x): x is string | TypedVar =>
              isIdentOrTypedVar(x)
            )
          )
        ) {
    ...
    const fv = typeof f === "string" ? localVars[f] : evalSExpr(f, localVars);
    if (isFunction(fv)) {
  ...
  function handleDefine(s: SExpr, localVars: Values): Values {
    ...
    const name = isTypedVar(varName) ? varName.name : varName;
  ...
  function evaluateAtom(a: Atom, localVars: Values): Value {
    ...
    const val =
      localVars[a] ??
      (mathPrimitives.includes(a as MathPrimitive)
        ? {
            type: "primitive-function",
            identifier: a,
          }
  ...
  function evalMathPrimitive(
    ...
    function dist(args: SExpr[], fn: (a: Value, b: Value) => boolean): boolean {
      ...
      return (
        fn(evalSExpr(x, localVars), evalSExpr(y, localVars)) &&
        dist([y, ...rest], fn)
      );
    }
